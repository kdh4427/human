...
public class BoardDao {
	...
	public Board findBy(Long bno) {
		return template.queryForObject("SELECT * FROM TBL_BOARD WHERE BNO = ?", new BoarMapper(), bno);
		
	}
	
	public List<Board> getListBoard() {
		return template.query("SELECT * FROM TBL_BOARD WHERE ROWNUM <= 10 ORDER BY BNO DESC", new BoarMapper());
	}
	
	private class BoarMapper implements RowMapper<Board> {
		@Override
